This repository contains source code implementing the RSA encryption algorithm. It further contains a custom BigInt class which may be used for arbitrary precision integer arithmetic (and is also required by the RsaInstance class as the algorithm uses 10-digit primes but could be increased to any number of digits).
...
I tested this program recently on Fedora Workstation 43 on my laptop, which works sufficiently quick with 10-digit primes.
...
1. The user is prompted to input a message which should be encrypted (you can input any UTF-16 character).
